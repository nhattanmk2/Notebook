...
public class NotStudiedAdapter extends RecyclerView.Adapter<NotStudiedAdapter.CourseViewHolder> implements Filterable {

    private Context mContext;
    private List<Subject> mListCourse;
    List<Subject> mListCourseRoot;

    public NotStudiedAdapter(Context mContext, List<Subject> mListCourse) {
    ...
    @NonNull
    @Override
    public NotStudiedAdapter.CourseViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_all_course, parent, false);
        return new NotStudiedAdapter.CourseViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull NotStudiedAdapter.CourseViewHolder holder, int position) {
